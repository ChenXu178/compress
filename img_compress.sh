...
###   -p, o0 - o7		png图片压缩率 数值大压缩率越高，默认从环境变量中读取，如果未设置则默认o3。
...
function find_img(){
	if [ $COMPRESS_JPG -eq 1 ];then
		#......jpegoptim.......jpg
		find "$1" -size +$2 -name '*.jpg' -print0 | parallel -0 -X jpegoptim --strip-all -m $MY_JPG_QUALITY {};
		find "$1" -size +$2 -name '*.JPG' -print0 | parallel -0 -X jpegoptim --strip-all -m $MY_JPG_QUALITY {};
		find "$1" -size +$2 -name '*.jpeg' -print0 | parallel -0 -X jpegoptim --strip-all -m $MY_JPG_QUALITY {};
		find "$1" -size +$2 -name '*.JPEG' -print0 | parallel -0 -X jpegoptim --strip-all -m $MY_JPG_QUALITY {};
	fi
    if [ $COMPRESS_PNG -eq 1 ];then
		#......optipng.......png....bmp
		find "$1" -size +$2 -name '*.png' -print0 | parallel -0 -X optipng -$MY_PNG_QUALITY {};
		find "$1" -size +$2 -name '*.PNG' -print0 | parallel -0 -X optipng -$MY_PNG_QUALITY {};
		find "$1" -size +$2 -name '*.bmp' -print0 | parallel -0 -X optipng -$MY_PNG_QUALITY {};
		find "$1" -size +$2 -name '*.BMP' -print0 | parallel -0 -X optipng -$MY_PNG_QUALITY {};
	fi
}
function show_config(){
	if [[  $COMPRESS_JPG -eq 1 && $COMPRESS_PNG -eq 1 ]];then
		...
		echo -e "\033[44;37m png 压缩率为(o0-o7)：$MY_PNG_QUALITY \033[0m"
		...
		echo -e "\033[44;37m 压缩率为(o0-o7)：$MY_PNG_QUALITY \033[0m"
	fi
...
function start_compress(){
	...
	echo -e "\033[32m \n压缩完成！原始大小：$oldsize，压缩后大小：$nowsize，$startTime ---> $endTime 总耗时：$ans\n \033[0m"
}
